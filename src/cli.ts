...
async function main(argv = process.argv.slice(2)) {
  ...
  const srv = (createInspector as any)({ port, quiet, token, allowOrigin, allowOriginStrict, devClient, host, cwd })
    ...
    if (!quiet) {
      ...
      if (token) authModes.push('legacy token enabled')
  ...
  if (cmd === 'session') {
    ...
    const httpRequest = (opts: { method: string; path: string; body?: any; token?: string }): Promise<{ status: number; body: string }> => {
      return new Promise((resolve, reject) => {
        const bodyStr = opts.body ? JSON.stringify(opts.body) : undefined
        const headers: Record<string, string> = { 'Content-Type': 'application/json' }
        if (opts.token) headers['x-fortistate-token'] = opts.token
