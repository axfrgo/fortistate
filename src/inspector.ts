...
export function createInspectorServer(opts: { port?: number; quiet?: boolean; token?: string; allowOrigin?: string; allowOriginStrict?: boolean } = {}) {
    ...
    start: async () => {
      server = http.createServer((req, res) => {
        ...
        if (req.method === 'OPTIONS' && (req.url === '/register' || req.url === '/change')) {
          ...
          const headers: Record<string,string> = { 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, x-fortistate-token' }
          if (allowOriginOption) headers['Access-Control-Allow-Origin'] = allowOriginOption
