...
export function createJitServer(opts: JitServerOptions = {}) {
  ...
  return {
    start: async () => {
      ...
      // keep unsub fns so stop can cleanup
    ...
    stop: async () => {
      ...
      try {
        wss!.clients.forEach((c: any) => {
          try { if (typeof c.terminate === 'function') c.terminate(); else if (typeof c.close === 'function') c.close() } catch (e) { }
        })
      } catch (e) { }
